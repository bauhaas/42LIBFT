...
for file in ${FILES[@]}; do
	...
	printf " ${CYAN}${file} ${GREEN}\033[${MAX_LENGHT_FILE_NAME}G\n"
	else
	((FILES_TOT=FILES_TOT+1))
	printf " ${CYAN}${file}  ${RED}\033[${MAX_LENGHT_FILE_NAME}G\n"
	printf " ${WHITE}$(norminette ${file} | grep "Error\|Warning" | cut -d " " -f2-)\n"
	fi
done
fi
#
